...
function SkillTile({ skill }: { skill: Skill }) {
  return <article className="skill-tile"><span className="skill-mark">{iconMap[skill.iconKey.toLowerCase()] ?? "<>"}</span><div><h3>{skill.name}</h3><p>{skill.description}</p></div><span className="skill-category">{skill.category}</span></article>
}
...
export default function Home() {
  ...
  return <main>
    ...
    <section id="projects" className="section-shell projects-section"><div className="section-intro"><div><p className="eyebrow">02 / selected work</p><h2>Things I&apos;ve built.</h2></div><span className="section-count">{String(projects.length).padStart(2, "0")} projects</span></div><div className="project-grid">{projects.map((project, index) => <Link href={`/projects/${slugify(project.title, project.id)}`} className="project-card" key={project.id}><div className={`project-art art-${index % 3}`}>{project.imageUrl ? <img src={project.imageUrl} alt="" /> : <><span className="art-grid" /><span className="art-label">{String(index + 1).padStart(2, "0")}</span><ArrowUpRight /></>}</div><div className="project-info"><div><p className="project-index">0{index + 1}</p><h3>{project.title}</h3><p>{project.description}</p></div><span className="card-arrow"><ArrowUpRight /></span></div><div className="tag-row">{project.tags.split(",").map((tag) => <span key={tag}>{tag.trim()}</span>)}</div></Link>)}</div></section>
